...
if __name__ == '__main__':
	...
	broken_links = []
	for c in db.session.query(Citation).join(CitationView).filter(CitationView.xpath_failure==True).filter(Citation.created >= day_ago):
	...
	html = template.render(since=day_ago.strftime('%d %B, %Y'), new_users=new_users, new_citations=new_citations, broken_links=broken_links)
	mail.send('"TLDRify" <webmaster@tldrify.com>', ['spektom@gmail.com'], 'tldrify.com - Daily Report', html=html)
